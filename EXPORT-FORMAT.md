...
`bands[].columns[]` of measured values, plus `jobNumber`, `machine`, `author`,
`editDate`, and `gageId` per band.

Exporting those is a strictly additive change under §5 — new columns at the
right end — but the measured values are two-dimensional (band × column) and do
not fit one-dataset-one-column without a flattening convention. Worth deciding
