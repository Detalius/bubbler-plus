...
// Prototype for the Excel export (EXPORT-FORMAT.md). Runs in the MAIN process:
// it needs fs and it must never block the renderer on a large package.
//
// The workbook is treated as what it is: a zip of XML parts. Only the four
// parts that MUST change are rewritten --
...
// -- and every other part is copied through byte for byte. vbaProject.bin,
// printer settings, styles, the form sheets themselves and anything a future
// Excel invents are carried untouched because they are never parsed.
//
// Values are written as inline strings (t="inlineStr"). That is the typing
// guarantee: a string in the file is a string on load, so Excel cannot decide
// that 1/4-20 is a date or that .250 is 0.25. It also means sharedStrings.xml
// is never touched, which is one fewer shared part to corrupt.
//
// No styles are referenced (s is left default), so xl/styles.xml is untouched
// and the appended sheet inherits the workbook's General format. Since every
// value is already a string, General displays it verbatim.
...
// Excel's own rules: 31 characters, no : \ / ? * [ ], no leading or trailing
// apostrophe, not empty. Enforced here so a sheet name that is legal in
// Bubbler+ cannot produce a workbook Excel refuses to open.
...
// Older data is never quietly overwritten: a collision takes a suffix instead.
// The base is trimmed so that name + suffix still fits in 31.
...
// columns: array of { header, values[] }. Ragged by design -- a one-value
// dataset is one cell, a 186-value dataset is 186. Short columns are simply
// absent below their last value; they are not padded, because an absent cell
// and an empty cell read the same and padding would bloat the part.
...
function appendSheets(bytes, sheets) {
  ...
  // Rebuilt in the original part order so the package looks as close to
  // untouched as a zip can. [Content_Types].xml stays first.
  const out = {};
  let untouched = 0;
